...
struct PlayerProjection
{
  ...
  static auto project(const Entity::PositionChanged& event)
  {
    return [event](auto& gui) {
      sf::Sprite ship;
  ...
  static auto project(const Entity::RotationChanged& event)
  {
    return [event](Gui& gui) {
...
struct MapProjection
{
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Apply MapUpdated
  static auto project(const MapUpdated& event)
  {
    return [event](auto& gui) {
      gui.map.clear();
          ...
          for (auto x = 0; x < row.size(); ++x) {
            ...
            if (cell) {
              ...
              sf::RectangleShape sprite(
                { (float)CreateMap::cell_width * scale_factor, (float)CreateMap::cell_width * scale_factor });
              sprite.move({ px, py });
...
void
serialize(Archive& ar, Player::Created& evt, const unsigned int version)
{
  ar& evt.player_id;
}
...
main()
{
  ...
  auto ctx = event_sauce::context<Gui,
                                  Player,
                                  PlayerProjection<Gui>,
                                  CameraProjection<Gui>,
                                  Map,
                                  MapProjection<Gui>,
                                  Entity,
                                  Time,
  ...
  while (gui.window.isOpen()) {
    ...
    const auto mouse_position = gui.window.mapPixelToCoords(sf::Mouse::getPosition(gui.window));
